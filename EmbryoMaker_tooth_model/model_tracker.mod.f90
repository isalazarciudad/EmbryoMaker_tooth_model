...
real*8::rad,nrad,ax,ay,az
  ...
  do  !*******this loop does the number of iterations equat to tf
    ...
    if (mod(getot,100).eq.0) then
      ...
      !print*,getot,"acumulated stress",a+b,"system stress per node", (a+b)/real(na+nb)
      !print*,getot,"epi acumulated stress",a,"epithelium stress per node", a/real(na)
      !print*,getot,"stellate acumulated stress",b,"stellate stress per node", b/real(nb)


      !!!!!!!UPDATING THE CELL TRACKING VECTORS  !!TRACKER MOD
      track_length=track_length+1

      do i=1,ncels
        k=cels(i)%node(1)
        cell_track(i,track_length,1)=node(k)%x
        cell_track(i,track_length,2)=node(k)%z
      end do
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
