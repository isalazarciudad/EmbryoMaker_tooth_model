...
subroutine inivisualitzacio
  ...
  flag(25)=conf_flag(25)!centroids 
  flag(26)=conf_flag(26)!cylinders 
...
real*8::fmax,fmin !!!! MECHVEC MOD

integer::epi_mes,mes_mes,total_mes
...
!    end select
  ...
  if (flag(25)==1) then			!>>>>>>>>>>>>>>>>>>>>>>>>>> Miquel 20-6-13
    !a=1.0d0 !this will be the max vector module plotted, vectors longer than this will be capped
    do i=1,nd
      a=1.0d0
      aa=vtorx(i) ; bb=vtory(i) ; cc=vtorz(i)
      d=sqrt(aa**2+bb**2+cc**2)
      color=0 ; color(1)=1.0 ; color(2)=0.0
      if (d>a) then ;
        !print*,"nod",i
        color=0d0 ; color(3)=1.0
        aa=(aa/d)*a ; bb=(bb/d)*a ; cc=(cc/d)*a
        !a=d ; ii=i ; 
      end if
    !end do
    aaa=10d0*esca/a !this way the vectors escalate so the greatest force is plotted with a constant length
    ...
    !do i=1,nd
      ...
      !if(node(i)%tipus==3.and.flag(31)==0) cycle
      !if(node(i)%tipus>3.and.flag(32)==0) cycle
      if(gex(i,3)>epsilod.and.flag(31)==0) cycle
      if(gex(i,2)>epsilod.and.flag(32)==0) cycle
      if(node(i)%hold==1 .and. flag(36)==0) cycle !>>Miquel22-1-14
      a=node(i)%x ; b=node(i)%y ; c=node(i)%z
      ...
      !  call glvertex3d(a*esca,b*esca,c*esca)
      !  call glvertex3d(aa*esca,bb*esca,cc*esca)
        call arrows(a,b,c,aa,bb,cc,color)
      end if
    end do
  end if
  ...
  if (flag(26)==1) then			!>>>>>>>>>>>>>>>>>>>>>>>>>> Miquel 20-6-13
    ...
    do i=1,nd
      ...
      d=sqrt(aa**2+bb**2+cc**2)
      color=0 ; color(1)=1.0 ; color(2)=1.0
      if (d>a) then ;
        !print*,"nod",i
        color=0d0 ; color(3)=1.0
        aa=(aa/d)*a ; bb=(bb/d)*a ; cc=(cc/d)*a
...
!!print *,i,node(i)%altre,node(i)%tipus
  ...
  if(custom_colorselection==colorselection)then
    ...
    !case(3) ; mine=-maxval(node(:nd)%z) ; maxe=-minval(node(:nd)%z)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  epi_mes=0 ; mes_mes=0 ; total_mes=0 !MECHVEC MOD: COUNTING epithelial-mesenchymal contacts
...
!print*,"arrowselection",arrowselection
  ...
  if (flag(40)==1) then
    ...
    if ((flag(30)==1.and.node(i)%tipus<3).or.(flag(31)==1.and.gex(i,3)>epsilod).or.(flag(32)==1.and.gex(i,2)>epsilod)) then
      if ((flag(9)==1.and.node(i)%tipus==1).or.(flag(10)==1.and.node(i)%tipus==2) .or.(node(i)%tipus>=3)) then      
        ...
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then 
          if (flag(22)==1.and.node(i)%tipus<3)then
            ...
            !  do j=1,nneigh(i)!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            !    ic=neigh(i,j)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            !    if(node(ic)%tipus<3)then!!!!!!!!!!!!!!!!!!!!!!!!

            !!!!!!!WE ARE SORTING MES CELLS BY THEIR CONTACT WITH EPITHELIUM!!!!
            !if(gex(i,3)>0.1)then!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            !      exit
            !    end if
            !    if(j==nneigh(i)) mes_mes=mes_mes+1
            !  end do
            !end if
  ...
  !print*,epi_mes, mes_mes, total_mes
  ...
  if(flag(12)==1.or.flag(13)==1)then   
    ...
    !fmax=maxval(fneigh) ; fmin=minval(fneigh) !MECHVEC MOD
    !fmax=0.05d0 ; fmin=-0.00d0 !MECHVEC MOD
    if(custom_colorselection==41)then
      fmax=custom_maxval ; fmin=custom_minval!minval(fneigh)
    else
      fmax=maxval(fneigh) ; fmin=minval(fneigh) !MECHVEC MOD
    end if

    !fmax=0.20d0 ; fmin=-0.20!minval(fneigh)
    ...
    do i=1,nd
      ...
      !if(node(i)%tipus==3.and.flag(31)==0) cycle
      !if(node(i)%tipus>3.and.flag(32)==0) cycle
      if(gex(i,1)>epsilod.and.flag(30)==0) cycle
      if(gex(i,3)>epsilod.and.flag(31)==0) cycle
      if(gex(i,2)>epsilod.and.flag(32)==0) cycle
      if(flag(36)==0.and.node(i)%hold==1) cycle  !>>>>Miquel15-1-14
      !if((flag(30)==1.and.node(i)%tipus<3).or.(flag(31)==1.and.node(i)%tipus==3).or.(flag(32)==1.and.node(i)%tipus==4))then
      if((flag(30)==1.and.gex(i,1)>epsilod).or.(flag(31)==1.and.gex(i,3)>epsilod).or.(flag(32)==1.and.gex(i,2)>epsilod))then
  ...
  !  if(node(ic)%tipus<3)then!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !  if(node(kkk)%tipus<3)then!!!!!!!!!!!!!!!!!!!!!!!!!!!
              ...
              if((flag(30)==1.and.gex(ic,1)>epsilod).or.(flag(31)==1.and.gex(ic,3)>epsilod).or.&
                                                         &(flag(32)==1.and.gex(ic,2)>epsilod))then
                if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                  d=dneigh(i,j)
                  if(d<node(i)%da+node(ic)%da)then     !>>>>>> Is 9-5-13
                    ...
                    d=fneigh(i,j)  !MECHVEC MOD
                    call get_rainbow(d,fmin,fmax,color)  !MECHVEC MOD
...
call glutAddMenuEntry("force component: Repulsion"//char(0),25)        !
call glutAddMenuEntry("force component: adhesion"//char(0),26)       !
