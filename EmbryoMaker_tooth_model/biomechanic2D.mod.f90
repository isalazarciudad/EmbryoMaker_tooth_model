     ...
     !if(nod==260.or.ic==260) print*,"dzero",nod,ic,fd
      ...
      if(ffu(4)==0 .and.twoep==1 ) then !it is only between epithelial nodes
        ...
        if(abs(dotp)-angletor*d>epsilod)then
          ...
          rstorx(nod)=rstorx(nod)+f*uvx ; rstorz(nod)=rstorz(nod)+f*uvz
          rstorx(ic)=rstorx(ic)-f*uvx ; rstorz(ic)=rstorz(ic)-f*uvz

          !parallel springs torsion   !new  !>>Miquel14-3-14
          dotp=(cx*ccx+cz*ccz)*udd !vertical projection, more stable than the angle
          f=(tornod+node(ic)%tor)*dotp
          uvx=ccx*ud ; uvz=ccz*ud
          rtorx(nod)=rtorx(nod)+f*uvx ; rtorz(nod)=rtorz(nod)+f*uvz
          ...
          f=(tornod+node(ic)%tor)*dotp
          rtorx(ic)=rtorx(ic)-f*uvx ; rtorz(ic)=rtorz(ic)-f*uvz
        end if
