...
integer,public, parameter      ::nfu=25              !CHECK MANUALLY number of basic forces !miguel4-11-13
...
real*8,public                  ::k_bu    !SCALE MOD
real*8,public                  ::m_xwall,m_ywall    !SCALE MOD
real*8,public                  ::mi_xwall,mi_ywall    !SCALE MOD
integer,public                 ::nborders    !SCALE MOD
real*8,public                  ::k_press !parameter controlling the effect of mechanical forces on cell cycle  !SCALE MOD
...
integer, public, allocatable   ::borders_neigh(:,:)  !an auxiliary neighbor matrix for periodic boundary conditions !ECTO_MOD
...
real*8, public, allocatable    ::cell_track(:,:,:) !stores the position for each cell at different time points  !!!!!! TRACKER MOD
integer, public                ::track_length !keeps track of the number of time intervals that have been stored
...
subroutine iniarrays
   ...
   !only necessary when using simple boxes neighbor search
    ...
    !setting all node and cell variables to 0  !>>Miquel28-1-14
     ...
     allocate(cell_track(1000,500,2)) !no dynamic reallocation, we have well defined ranges for what we want to do !!TRACKER MOD
     track_length=0 !!TRACKER MOD
