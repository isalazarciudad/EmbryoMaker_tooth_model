...
private
      public :: view_modifier_init,reset_view,arrow_key_func,mouseswitch,pressed_left,switch,& !Tommi
                reset_to_init,colorswitch,view_from_front !Tommi 13.8.2013
private :: ZOOM, PAN, ROTATE, SCALEX, SCALEY, SCALEZ, RESET, ABOVE, QUIT, PI, &
            middle_button_func,&
...
anglep%x = 0.0_gldouble*slookfrom%theta/PI !- 30.0_gldouble			!display, angle of camera respect the origin
anglep%y = 0.0_gldouble*slookfrom%phi/PI - 45.0_gldouble
  ...
  if (button == GLUT_MIDDLE_BUTTON .and. state == GLUT_UP) then
    moving_middle = .false.
	  ...
	  pressed_left = .true.
          ...
          select case (plane)
          case(1) !selection plane is miy
	   ycoord=miy
	   xcoord = startxmax - ((ymax-xcoordmous)*yconver) 
	   !if (xcoord > startxmax) then
	    !  xcoord = startxmax
	   !else if (xcoord < startxmin) then
	    !  xcoord = startxmin
	   !endif
	   zcoord = startymin +((zmax-ycoordmous)*zconver)
	   !if (zcoord > startymax) then
	    !  zcoord = startymax
	   !else if (zcoord < startymin) then
	    !  zcoord = startymin
	   !endif
          case(2) !selection plane is mx
           xcoord=mx
	   ycoord = startxmax - ((ymax-xcoordmous)*yconver) 
	  ! if (ycoord > startxmax) then
	   !   ycoord = startxmax
	   !else if (ycoord < startxmin) then
	    !  ycoord = startxmin
	   !endif
	   zcoord = startymin +((zmax-ycoordmous)*zconver)
	   !if (zcoord > startymax) then
	   ...
	   !else if (zcoord < startymin) then
	    !  zcoord = startymin
	   !endif
          case(3) !selection plane is mix
           xcoord=mix
	   ycoord = startxmin + ((ymax-xcoordmous)*yconver) 
           !print*,ycoord,"ycoord"
	   !if (ycoord < startxmin) then
	    !  ycoord = startxmin
	   !else if (ycoord > startxmax) then
	    !  ycoord = startxmax
	   !endif
	   zcoord = startymin +((zmax-ycoordmous)*zconver)
	   !if (zcoord > startymax) then
	   ...
	   !else if (zcoord < startymin) then
	    !  zcoord = startymin
	   !endif
          case(4) !selection plane is my
           ycoord=my
	   xcoord = startxmin + ((ymax-xcoordmous)*yconver) 
	   !if (xcoord < startxmin) then
	    !  xcoord = startxmin
	   !else if (xcoord > startxmax) then
	    !  xcoord = startxmax
	   !endif
	   zcoord = startymin +((zmax-ycoordmous)*zconver)
	   !if (zcoord > startymax) then
	   ...
	   !else if (xcoord < startxmin) then
	    !  xcoord = startxmax
	   !endif
	   ycoord = startymin + ((zmax-ycoordmous)*zconver)
	   !if (ycoord > startymax) then
	    !  ycoord = startymax
	   !else if (ycoord < startymin) then
	    !  ycoord = startymin
	   !endif
          case(6) !selection plane is miz
           zcoord=miz
	   xcoord = startxmax - ((ymax-xcoordmous)*yconver) 
	   !if (xcoord > startxmax) then
	    !  xcoord = startxmax
	   !else if (xcoord < startxmin) then
	    !  xcoord = startxmax
	   !endif
	   ycoord = startymax - ((zmax-ycoordmous)*zconver)
          end select
	  endif
	  select case (choice)
          case(1)
              call addnode (xcoord,ycoord,zcoord)
          case(2)
	  case(7)
              call pastenode (xcoord,ycoord,zcoord)
           case(6)
              call selectnode (xcoord,ycoord,zcoord)
              do i=1,nda
              node(i)%marge=0 !!!!!!ATTENTION, marge is being set to zero
              end do
              node(nodeindex)%marge=52
          case(8)
              call addcell(xcoord,ycoord,zcoord,plane)
               select case(make_flag_twe_five)
		     case(1)!selection plane is mx
		       mx=maxval(node(:nd)%x)
		     case(2)!selection plane is mix
		       mix=minval(node(:nd)%x) 
		     case(3)!selection plane is my
		       my=maxval(node(:nd)%y)
		     case(4)!selection plane is miy
		       miy=minval(node(:nd)%y) 
		     case(5)!selection plane is mz
		       mz=maxval(node(:nd)%z)
		     case(6)!selection plane miz
		       miz=minval(node(:nd)%z)
		end select
          case(9)
              call selectcell(xcoord,ycoord,zcoord)
              do i=1,nda
              node(i)%marge=0 !!!!!!ATTENTION, marge is being set to zero
              end do
              do i=1,cels(cellid)%nunodes
              kk=cels(cellid)%node(i)
              node(kk)%marge=52
              end do
	  case(10)
              call pastecell(xcoord,ycoord,zcoord,plane,pasteselection) !Tommi 27.9.2013
               select case(make_flag_twe_five)
		     case(1)!selection plane is mx
		       mx=maxval(node(:nd)%x)
		     case(2)!selection plane is mix
...
if(switch) then
left_button_func = PAN !Button function can be changed from the editor also
endif
...
case (9)
  ! copy=mx 
   ...
   mx=mx+extre*mesc*(x-begin%x)
   !mx=9.1389584338
   print*,mx,"mx"
   tempo=(extre*mesc*(x-begin%x))/1.0
   if (mx >3.1389584338) then
   converymax=tempo*8.8333333333
   converyconver=tempo*(-0.0002190013)
   if (mx > 4.1389584541) then
   converzconver=tempo*(-0.0002258356)
   else
   converzconver=tempo*(-8.65351407407424E-005)
   endif
...
case (10)
   ...
   miy=miy+extre*mesc*(y-begin%y)
   !miy=-8.071971135656904
   tempo=(extre*mesc*(y-begin%y))/1.0
   if (miy < -3.0719711357) then
...
case (11)
   ...
   my=my+extre*mesc*(y-begin%y)
   !my=-0.9280288643
   tempo=(extre*mesc*(y-begin%y))/1.0
   if (my >3.0719711357) then
   converymax=tempo*8.6
   converyconver=tempo*(-0.0002208942)
...
!          ------------
integer(kind=glint), intent(in), value :: value
...
call glutAddMenuEntry("minimal x plane"//char(0),8)
call glutAddMenuEntry("maximal x plane"//char(0),9)
call glutAddMenuEntry("minimal y plane"//char(0),10)
call glutAddMenuEntry("maximal y plane"//char(0),11)
call glutAddMenuEntry("minimal z plane"//char(0),12)
call glutAddMenuEntry("maximal z plane"//char(0),13)
call glutAddMenuEntry("proximal clipping plane"//char(0),14)
call glutAddMenuEntry("distal clipping  plane"//char(0),15)
call glutAddMenuEntry("change the scale of mouse escale for clipping"//char(0),16)
...
implicit none
private
public :: display,Reshape,draw_func,menu_handler,make_menu,inivisualitzacio,menuu_handler,menud_handler,menut_handler, &
          menuq_handler,menus_handler,menuse_handler, menueditor_handler, planemenu_handler,nodemenu_handler,cellmenu_handler, &
          nodecopymenu_handler,cellcopymenu_handler !Tommi 23.9.2013
...
integer,private                ::ki,nki
integer,private,allocatable    ::oopp(:)      !warning: arbitrary size
...
subroutine inivisualitzacio
  colorselection=29
  iqpas=1
  flag(1)=1 !elipse lines ATTENTION 0!!! Tommi 
  flag(2)=0 !boxes
  flag(3)=0 !cell polygons
  flag(4)=0 !cell polygon sides
  flag(5)=0 !balls req					!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
  flag(6)=1 !balls da					!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
  flag(7)=0 !marked nodes
  flag(8)=0 !no balls
  flag(9)=1 !upper balls
  flag(10)=1!lower balls 
  flag(11)=0!small balls
  flag(12)=0!connexions between cells
  flag(18)=1!main box
  flag(19)=0!polarization vectors 
  flag(20)=0! 
  flag(22)=0!cylinders 
  flag(21)=0!centroids 
  flag(30)=1!epithelium
  flag(31)=1!mesenchyme
  flag(32)=1!ecm
  flag(33)=0!full polygons
  flag(34)=0
  flag(35)=0!eggshell !miguel4-11-13
  flag(36)=1!plot the "hold" nodes (node()%hold=1) !>>>>Miquel15-1-14
  flag(37)=0!plot the whole triangulation  >>>>Miquel27-2-14
  flag(38)=0!plot boundary faces  >>>>Miquel28-2-14
  flag(39)=0!plot voronoi diagramm  >>>>Miquel4-3-14
  !plotting window
  
  mix=minval(node(:nd)%x) ; mx=maxval(node(:nd)%x)
  miy=minval(node(:nd)%y) ; my=maxval(node(:nd)%y)
!  miy=0d0 ; my=maxval(node(:nd)%y)                   !uncomment this to make a slice on the center
 miz=minval(node(:nd)%z) ; mz=maxval(node(:nd)%z)
  mesc=1d-3
  maan=0
  mapo=2500
  chogen=0
...
subroutine takeimage
  ...
  character*30 nomfitx,nofi
    ...
    print *,winame,"winame"
    pimp="import -window  "//winame//" kk.gif" 
    pimp(16:16)='"'
    pimp(55:55)='"'
    print *,pimp
    call system(pimp)
    call system("mv kk.gif images/"//nomfitx//nofi) 
end subroutine
...
!   call glvertex3d(0.0_gldouble,0.0_gldouble,3*esca)
  ...
  if (flag(18)==1) then
    ...
    if (.not.colorswitch) then !>>>>>>Tommi !Give control of the main box to the editor, if editor not in use, then normal box
      call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,verd)
    ...
    else !if in use, draw main box normally
      call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,verd)
      call glvertex3d(mix*esca,miy*esca,miz*esca) ; call glvertex3d(mx*esca,miy*esca,miz*esca)
      call glvertex3d(mix*esca,miy*esca,miz*esca) ; call glvertex3d(mix*esca,my*esca,miz*esca)
      call glvertex3d(mix*esca,my*esca,mz*esca)   ; call glvertex3d(mix*esca,my*esca,miz*esca)
      ...
      select case(plane) !and the selection window (white) in the chosen plane !Tommi 11.9.2013
      case(1)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(startxmax*esca,miy*esca,startymax*esca)  ; call glvertex3d(startxmax*esca,miy*esca,startymin*esca) !xmax, miny
        call glvertex3d(startxmin*esca,miy*esca,startymin*esca)  ; call glvertex3d(startxmax*esca,miy*esca,startymin*esca) !miny
        call glvertex3d(startxmin*esca,miy*esca,startymax*esca)  ; call glvertex3d(startxmin*esca,miy*esca,startymin*esca) !miny
        call glvertex3d(startxmin*esca,miy*esca,startymax*esca)  ; call glvertex3d(startxmax*esca,miy*esca,startymax*esca) !miny
      case(2)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(mx*esca,startxmin*esca,startymin*esca)   ; call glvertex3d(mx*esca,startxmax*esca,startymin*esca) !xmax
        call glvertex3d(mx*esca,startxmin*esca,startymax*esca)   ; call glvertex3d(mx*esca,startxmin*esca,startymin*esca) !xmax
	    call glvertex3d(mx*esca,startxmin*esca,startymax*esca)   ; call glvertex3d(mx*esca,startxmax*esca,startymax*esca) !xmax
        call glvertex3d(mx*esca,startxmax*esca,startymax*esca)   ; call glvertex3d(mx*esca,startxmax*esca,startymin*esca) !xmax
      case(3)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(mix*esca,startxmax*esca,startymax*esca) ; call glvertex3d(mix*esca,startxmin*esca,startymax*esca) !minx
        call glvertex3d(mix*esca,startxmin*esca,startymin*esca) ; call glvertex3d(mix*esca,startxmin*esca,startymax*esca) !minx
        call glvertex3d(mix*esca,startxmin*esca,startymin*esca) ; call glvertex3d(mix*esca,startxmax*esca,startymin*esca) !minx
        call glvertex3d(mix*esca,startxmax*esca,startymin*esca) ; call glvertex3d(mix*esca,startxmax*esca,startymax*esca) !minx
      case(4)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(startxmax*esca,my*esca,startymax*esca)  ; call glvertex3d(startxmax*esca,my*esca,startymin*esca) !maxy
        call glvertex3d(startxmax*esca,my*esca,startymin*esca)  ; call glvertex3d(startxmin*esca,my*esca,startymin*esca) !maxy
        call glvertex3d(startxmax*esca,my*esca,startymax*esca)  ; call glvertex3d(startxmin*esca,my*esca,startymax*esca) !maxy
        call glvertex3d(startxmin*esca,my*esca,startymax*esca)  ; call glvertex3d(startxmin*esca,my*esca,startymin*esca) !maxy
      case(5)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(startxmin*esca,startymin*esca,mz*esca)  ; call glvertex3d(startxmax*esca,startymin*esca,mz*esca)  !maxz
        call glvertex3d(startxmin*esca,startymax*esca,mz*esca)   ; call glvertex3d(startxmin*esca,startymin*esca,mz*esca) !maxz
        call glvertex3d(startxmin*esca,startymax*esca,mz*esca)   ; call glvertex3d(startxmax*esca,startymax*esca,mz*esca) !maxz
        call glvertex3d(startxmax*esca,startymin*esca,mz*esca)   ; call glvertex3d(startxmax*esca,startymax*esca,mz*esca) !maxz
      case(6)
        call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,white) !color the desired plane differently for the editor
        call glvertex3d(startxmin*esca,startymax*esca,miz*esca) ; call glvertex3d(startxmax*esca,startymax*esca,miz*esca) !minz
        call glvertex3d(startxmin*esca,startymax*esca,miz*esca) ; call glvertex3d(startxmin*esca,startymin*esca,miz*esca) !minz
        call glvertex3d(startxmin*esca,startymin*esca,miz*esca)  ; call glvertex3d(startxmax*esca,startymin*esca,miz*esca)!minz
        call glvertex3d(startxmax*esca,startymax*esca,miz*esca)  ; call glvertex3d(startxmax*esca,startymin*esca,miz*esca)!minz
      end select  	     
    endif
  else
    mix=minval(node(:nd)%x) ; mx=maxval(node(:nd)%x)   !>>>>Miquel10-12-13
    miy=minval(node(:nd)%y) ; my=maxval(node(:nd)%y)
    miz=minval(node(:nd)%z) ; mz=maxval(node(:nd)%z)
  end if !<<<<<<<Tommi
  ...
  if(flag(2)==1)then
    ...
    call glBegin(gl_lines)
    call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
    do j=-scale,scale															!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
      do k=-scale,scale														!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        a=-d*(scale+0.5d0);aa=d*(scale+0.5d0)										!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        b=d*(j+0.5d0)
        c=d*(k+0.5d0)
    ...
    do i=-scale,scale															!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
      do k=-scale,scale														!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        a=d*(i+0.5d0)
        b=-d*(scale+0.5d0);bb=d*(scale+0.5d0)										!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        c=d*(k+0.5d0)
    ...
    do i=-scale,scale															!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
      do j=-scale,scale														!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        a=d*(i+0.5d0)
        b=d*(j+0.5d0)
        c=-d*(scale+0.5d0);cc=d*(scale+0.5d0)										!>>>>>>>>>>>>>>>>>>>MIQUEL 4-3-13
        call glvertex3d(esca*a,esca*b,esca*c)
  ...
  case(24) ; mine=minval(node(:nd)%diffe) ; maxe=maxval(node(:nd)%diffe)
  case(25) ; mine=minval(node(:nd)%khold) ; maxe=maxval(node(:nd)%khold)
  case(26) ; mine=minval(node(:nd)%kplast) ; maxe=maxval(node(:nd)%kplast)
  case(27) ; mine=minval(node(:nd)%kvol) ; maxe=maxval(node(:nd)%kvol)
  case(28) ; mine=1 ;maxe=7 !mine=minval(node(:nd)%tipus) ; maxe=maxval(node(:nd)%tipus) !this is tipus
  case(29) ; mine=minval(node(:nd)%icel) ; maxe=maxval(node(:nd)%icel)
  case(30) ; mine=minval(node(:nd)%altre) ; maxe=maxval(node(:nd)%altre)
  case(31) ; mine=minval(node(:nd)%marge) ; maxe=maxval(node(:nd)%marge) 
  case(32) ; mine=minval(node(:nd)%talone) ; maxe=maxval(node(:nd)%talone) 
  case(33) ; mine=0;maxe=1!minval(node(:nd)%hold) ; maxe=maxval(node(:nd)%hold) 
  case(34) ; mine=0;maxe=1!minval(node(:nd)%border) ; maxe=maxval(node(:nd)%border) 
  case(35) ; mine=minval(gex(:nd,1)) ; maxe=maxval(gex(:nd,1));print*,"mine",mine,"maxe",maxe !; do i=1,nd;print *,i,gex(i,1),node(i)%x;end do
             mine=0d0 !;print*,"mine",mine,"maxe",maxe
             mine=0.0  !; maxe=0.1d0
  case(36) ; mine=minval(gex(:nd,2)) ; maxe=maxval(gex(:nd,2))
             mine=0d0
             mine=0.0  !; maxe=0.01d0
  case(37) ; if (chogen==0) then
             print *,"which gene" ; read(*,*) chogen ; print *,chogen,"chosen gene"
             mine=minval(gex(:nd,chogen)) ; maxe=maxval(gex(:nd,chogen))
             mine=0d0 ;print*,"maxe",maxe; end if
...
!  case(35) ; mine=minval(erep) ; maxe=maxval(erep)
!  case(36) ; mine=minval(erepcel) ; maxe=maxval(erepcel)
!  case(37) ; mine=minval(eyou) ; maxe=maxval(eyou)
!  case(38) ; mine=minval(eadh) ; maxe=maxval(eadh)
!  case(39) ; mine=minval(etor) ; maxe=maxval(etor)
!  case(40) ; mine=minval(espring) ; maxe=maxval(espring)
  end select 
  ...
  do i=1,nd
    ...
    case(24) ; a=node(i)%diffe
    case(25) ; a=node(i)%khold
    case(26) ; a=node(i)%kplast
    case(27) ; a=node(i)%kvol
    case(28) ; a=node(i)%tipus 
    case(29) ; a=node(i)%icel
    case(30) ; a=node(i)%altre
    case(31) ; a=node(i)%marge
    case(32) ; a=node(i)%talone
    case(33) ; a=node(i)%hold
    case(34) ; a=node(i)%border
    case(35) ; a=gex(i,1)
    case(36) ; a=gex(i,2)
    case(37) ; a=gex(i,chogen)
    case(38) ; a=disx(i)
    case(39) ; a=disy(i)
    case(40) ; a=disz(i)
    case(41) ; a=distot(i)
...
!    case(41) ; a=espring(i)
    ...
    call get_rainbow(a,mine,maxe,color)
    ...
    if ((flag(30)==1.and.node(i)%tipus<3).or.(flag(31)==1.and.node(i)%tipus==3).or.(flag(32)==1.and.node(i)%tipus==4)) then
      if ((flag(9)==1.and.node(i)%tipus==1).or.(flag(10)==1.and.node(i)%tipus==2) .or.(node(i)%tipus>=3)) then      
        ...
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then  
          ...
          call glTranslated(-a*esca,-b*esca,-c*esca)
          if (flag(22)==1.and.node(i)%tipus<3) call cylinder(i,20,color)   !>>>>>>>>>> Miquel 11-6-13
        endif
       end if
    end if
  end do
  ...
  if(flag(12)==1.or.flag(13)==1)then   !we draw the intercellular connexions with lines (nodes further than node()%da won't be connected)
    do i=1,nd
      if(flag(36)==0.and.node(i)%hold==1) cycle  !>>>>Miquel15-1-14
      if((flag(30)==1.and.node(i)%tipus<3).or.(flag(31)==1.and.node(i)%tipus==3).or.(flag(32)==1.and.node(i)%tipus==4))then
        a=node(i)%x ; b=node(i)%y ; c=node(i)%z
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
          if(ffu(13)==1)then
            do j=1,nneigh(i)
              ic=neigh(i,j)
              !if(ic==-1) cycle !this means it's a border node, only used in diffusion
              if((flag(12)==1.and.node(i)%icel/=node(ic)%icel).or.(flag(13)==1.and.node(i)%icel==node(ic)%icel))then
                ...
                if((flag(30)==1.and.node(ic)%tipus<3).or.(flag(31)==1.and.node(ic)%tipus==3).or.&
                  ...
                  if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                    ...
                    !if(d<rdiffmax.and.flag(37)==0.or.flag(37)==1)then
                      if (flag(12)==1) then
                        if(node(i)%tipus==1)then;color=0;color(1)=1
                        else;color=0;color(2)=1;end if
                      else
                        if(node(i)%tipus==1)then;color=0;color(2)=0.75
          ...
          else !;print*,"node i",i,nneigh(i),neigh(i,1:nneigh(i))
            do j=1,nneigh(i)
              ic=neigh(i,j)
              if((flag(12)==1.and.node(i)%icel/=node(ic)%icel).or.(flag(13)==1 .and. node(i)%icel==node(ic)%icel))then
                aa=node(ic)%x ; bb=node(ic)%y ; cc=node(ic)%z
  ...
  if(flag(37)==1 .or. flag(38)==1)then !we are calling the triangulation subroutine, whether we use it in the simulation or not !>>Miquel4-3-14
    npt=nd
    sizht=3/2*npt
    maxfc=npt**2
    maxbf=npt**2
    ...
    do i=1,nfc
      if(fc(1,i)>0.and.fc(2,i)>0.and.fc(3,i)>0)then !a valid triangle
        ...
        if(flag(37)==1)then  !triangulation lines
          if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
            if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
              call glBegin(gl_lines)
              color=0 ; color(2)=1
        ...
        if(flag(38)==1)then !border faces
          if(fc(5,i)<0)then !its a border face
            if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
  ...
  !>>>>>>>>>>>>BEGIN Is 24-9-13 modified Is 15-5-13
  call glBegin(gl_triangles)
   if(flag(33)==1)then   !FULL POLYGONS (CONEXIONS)
     do i=1,nd
       if(flag(36)==0.and.node(i)%hold==1) cycle  !>>>>Miquel15-1-14
       if (node(i)%tipus>2) cycle
       a=node(i)%x ; b=node(i)%y ; c=node(i)%z
       aaa=0 ; bbb=0; ccc=0        !first node
       if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
         iii=nint(a*urv) ; jjj=nint(b*urv) ; kkk=nint(c*urv)
         do ii=-1,1	!first search
           do jj=-1,1
             do kk=-1,1
               ic=boxes(iii+ii,jjj+jj,kkk+kk)
               do while(ic.ne.0)
                 if (node(i)%tipus/=node(ic)%tipus) then ; ic=list(ic) ; cycle ; end if
                 if(ic==i)then;ic=list(ic);cycle;end if
                 if(node(ic)%icel==node(i)%icel)then;ic=list(ic);cycle;end if
                 if(ic==node(i)%altre)then ;ic=list(ic);cycle;end if  !el vei el passem de llarg
                 aa=node(ic)%x ; bb=node(ic)%y ; cc=node(ic)%z !second node
                 d=sqrt((a-aa)**2+(b-bb)**2+(c-cc)**2)
                 if(d>node(i)%da+node(ic)%da)then;ic=list(ic);cycle;end if
                 lll=nint(aa*urv) ; mmm=nint(bb*urv) ; nnn=nint(cc*urv)
                 do ll=-1,1	!second search
                   do mm=-1,1
                     do nn=-1,1
                       icc=boxes(lll+ll,mmm+mm,nnn+nn)
                       do while(icc.ne.0)
                         if (node(i)%tipus/=node(icc)%tipus) then ; icc=list(icc) ; cycle ; end if
                         if(ic==i)then;ic=list(ic);cycle;end if
                         if(icc==node(i)%altre)then ;icc=list(icc);cycle;end if
                         aaa=node(icc)%x ; bbb=node(icc)%y ; ccc=node(icc)%z   !third node
                         if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.cc<=mz) then
                           dd=sqrt((a-aaa)**2+(b-bbb)**2+(c-ccc)**2)
                           if(dd>node(i)%da+node(icc)%da)then;icc=list(icc);cycle;end if
                           ddd=sqrt((aa-aaa)**2+(bb-bbb)**2+(cc-ccc)**2)
                           if(ddd>node(ic)%da+node(icc)%da)then;icc=list(icc);cycle;end if
                           normal=normcrossprod((/aaa,a,aa/),(/bbb,b,bb/),(/ccc,c,cc/)) !we've got a triangle, let's draw it
                           normal=abs(normal)
                           call glnormal3fv(normal)
  ...
  !>>>>>>>>>>>>BEGIN Miguel 25-9-13
  if(flag(34)==1)then  !POLYHEDRAL CELLS MESENCHYME
    do j=1,ncels
      if(flag(36)==0.and.node(cels(j)%node(1))%hold==1) cycle  !>>>>Miquel15-1-14
      if(cels(j)%ctipus.eq.3)then ! only mesenchymal cells
        call hull(j)
        do i=1,trmax
          ii=triang(i,1) ; jj=triang(i,2) ; kk=triang(i,3) 
          if((ii.ne.0).and.(jj.ne.0).and.(kk.ne.0))then  
            a=node(ii)%x  ; b=node(ii)%y  ;c=node(ii)%z   
            aa=node(jj)%x ; bb=node(jj)%y ;cc=node(jj)%z
            aaa=node(kk)%x; bbb=node(kk)%y;ccc=node(kk)%z
            if(ffu(6).eq.1)then                                                   !miguel27-11-13
              d=a-cels(j)%cex ; dd=b-cels(j)%cey ; ddd=c-cels(j)%cez          !miguel27-11-13
              fff=sqrt(d**2+dd**2+ddd**2)                                     !miguel27-11-13
              d=d/fff ; dd=dd/fff ; ddd=ddd/fff                               !miguel27-11-13
              a=a+node(ii)%req*d ; b=b+node(ii)%req*dd ; c=c+node(ii)%req*ddd !miguel27-11-13 

              d=aa-cels(j)%cex ; dd=bb-cels(j)%cey ; ddd=cc-cels(j)%cez       !miguel27-11-13
              fff=sqrt(d**2+dd**2+ddd**2)                                     !miguel27-11-13
              d=d/fff ; dd=dd/fff ; ddd=ddd/fff                               !miguel27-11-13
              aa=aa+node(jj)%req*d ; bb=bb+node(jj)%req*dd ; cc=cc+node(jj)%req*ddd !miguel27-11-13

              d=aaa-cels(j)%cex ; dd=bbb-cels(j)%cey ; ddd=ccc-cels(j)%cez    !miguel27-11-13
              fff=sqrt(d**2+dd**2+ddd**2)                                     !miguel27-11-13
              d=d/fff ; dd=dd/fff ; ddd=ddd/fff                               !miguel27-11-13
              aaa=aaa+node(kk)%req*d ; bbb=bbb+node(kk)%req*dd ; ccc=ccc+node(kk)%req*ddd !miguel27-11-13
            end if                                                               !miguel27-11-13
            if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
                  call glBegin(gl_triangles)		!MAKING THE SURFACE OF THE TRIANGLES
            ...
            !normal vectors    
            !a=(a+aa+aaa)/3d0 ; b=(b+bb+bbb)/3d0 ; c=(c+cc+ccc)/3d0
            !aa=a+trinor(i,1) ; bb=b+trinor(i,2) ; cc=c+trinor(i,3)
            do kkk=1,3
              if(kkk.eq.1)then
                a=node(ii)%x;b=node(ii)%y;c=node(ii)%z
                aa=node(jj)%x;bb=node(jj)%y;cc=node(jj)%z
              ...
              else if(kkk.eq.2)then;a=node(kk)%x;b=node(kk)%y;c=node(kk)%z
                aa=node(jj)%x;bb=node(jj)%y;cc=node(jj)%z 
                if(ffu(6).eq.1)then                                                    !miguel27-11-13
                  d=a-cels(j)%cex ; dd=b-cels(j)%cey ; ddd=c-cels(j)%cez		       !miguel27-11-13
                  fff=sqrt(d**2+dd**2+ddd**2)					       !miguel27-11-13
                  d=d/fff ; dd=dd/fff ; ddd=ddd/fff                                     !miguel27-11-13 
                  a=a+node(kk)%req*d ; b=b+node(kk)%req*dd ; c=c+node(kk)%req*ddd       !miguel27-11-13
              ...
              else if(kkk.eq.3)then;a=node(ii)%x;b=node(ii)%y;c=node(ii)%z     
                aa=node(kk)%x;bb=node(kk)%y;cc=node(kk)%z !; endif
                if(ffu(6).eq.1)then						       !miguel27-11-13
                  d=a-cels(j)%cex ; dd=b-cels(j)%cey ; ddd=c-cels(j)%cez                !miguel27-11-13
                  fff=sqrt(d**2+dd**2+ddd**2)                                           !miguel27-11-13
                  d=d/fff ; dd=dd/fff ; ddd=ddd/fff                                     !miguel27-11-13
                  ...
                  fff=sqrt(d**2+dd**2+ddd**2)                                           !miguel27-11-13
                  d=d/fff ; dd=dd/fff ; ddd=ddd/fff                                     !miguel27-11-13 
                  aa=aa+node(kk)%req*d ; bb=bb+node(kk)%req*dd ; cc=cc+node(kk)%req*ddd !miguel27-11-13
                endif                                                                 !miguel27-11-13
              endif
              if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
                call glBegin(gl_lines) ! making the edges of the triangles
                 color=0d0
                 call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
                 call glvertex3d(a*esca,b*esca,c*esca)
                 call glvertex3d(aa*esca,bb*esca,cc*esca)
          ...
          end if ! if triang(:,:).ne.0
        end do
      end if ! if cell is mesenchymal
    end do			
  end if

  !POLYGONS
  if (flag(3)==1) then		!we use the delaunay algorithm to paint the apical and basal surface
    !>>>>>>>>>>>>BEGIN Miquel 3-5-13
    do ik=1,ncels
      if(flag(36)==0.and.node(cels(ik)%node(1))%hold==1) cycle  !>>>>Miquel15-1-14
      if(cels(ik)%ctipus>=3) cycle
      call delaunay(ik)
      do j=1,trmax			!the delaunay is only made for one layer (the basal I guess)
        ii=triang(j,1);jj=triang(j,2);kk=triang(j,3) !miguel 15-5-13
        if(ii==0 .or. jj==0 .or. kk==0) exit
        a=node(ii)%x;b=node(ii)%y;c=node(ii)%z
        aa=node(jj)%x;bb=node(jj)%y;cc=node(jj)%z
        aaa=node(kk)%x;bbb=node(kk)%y;ccc=node(kk)%z
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
        ...
        ii=node(ii)%altre;jj=node(jj)%altre;kk=node(kk)%altre
        a=node(ii)%x;b=node(ii)%y;c=node(ii)%z	!here we repeat the process for the other layer
        aa=node(jj)%x;bb=node(jj)%y;cc=node(jj)%z
        aaa=node(kk)%x;bbb=node(kk)%y;ccc=node(kk)%z
        if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
          if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
  ...
  if(flag(4)==1)then	!we will use the delaunay to assess which nodes are on the borders of the cell
    do i=1,ncels
      if(flag(36)==0.and.node(cels(i)%node(1))%hold==1) cycle  !>>>>Miquel15-1-14
      if(cels(i)%ctipus>=3) cycle
      call delaunay(i)
      do j=1,cels(i)%nunodes		!the delaunay is only made for one layer (the basal I guess)
        jj=cels(i)%node(j)		!primer vei
        do k=1,cels(i)%nunodes
          kk=cels(i)%node(k)		!segon vei
          if(jj==kk)cycle
          ii=0
          do iiii=1,trmax!numnod !cels(i)%nutr
            iii=triang(iiii,1);jjj=triang(iiii,2);kkk=triang(iiii,3) !miguel 15-5-13
            if(jj==iii.or.jj==jjj.or.jj==kkk)then
              if(kk==iii.or.kk==jjj.or.kk==kkk)then
                ii=ii+1
              end if
            end if
          end do
          if(ii==1)then	!we found a border
            jjj=node(jj)%altre;kkk=node(kk)%altre
            aaa=0.25d0*(node(jj)%x+node(kk)%x+node(jjj)%x+node(kkk)%x)	!this is the midpoint of the side face (we have to make 4 triangles
            bbb=0.25d0*(node(jj)%y+node(kk)%y+node(jjj)%y+node(kkk)%y)
            ccc=0.25d0*(node(jj)%z+node(kk)%z+node(jjj)%z+node(kkk)%z)
            a=node(jj)%x;b=node(jj)%y;c=node(jj)%z
              ...
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
                  call glBegin(gl_triangles)
                   ...
                   call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
                   call glvertex3d(aa*esca,bb*esca,cc*esca)
                   color(:)=(memcolor(jj,:)+memcolor(kk,:)+memcolor(jjj,:)+memcolor(kkk,:))/4
                   call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
                   call glvertex3d(aaa*esca,bbb*esca,ccc*esca)
                  call glEnd
                  call glBegin(gl_lines)
            ...
            a=node(jj)%x;b=node(jj)%y;c=node(jj)%z
            aa=node(jjj)%x;bb=node(jjj)%y;cc=node(jjj)%z
            if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
                  call glBegin(gl_triangles)
            ...
            a=node(jjj)%x;b=node(jjj)%y;c=node(jjj)%z
            aa=node(kkk)%x;bb=node(kkk)%y;cc=node(kkk)%z
            if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
                  call glBegin(gl_triangles)
            ...
            a=node(kk)%x;b=node(kk)%y;c=node(kk)%z
            aa=node(kkk)%x;bb=node(kkk)%y;cc=node(kkk)%z
            if (a>=mix.and.a<=mx.and.b>=miy.and.b<=my.and.c>=miz.and.c<=mz) then
              if (aa>=mix.and.aa<=mx.and.bb>=miy.and.bb<=my.and.cc>=miz.and.cc<=mz) then
                if (aaa>=mix.and.aaa<=mx.and.bbb>=miy.and.bbb<=my.and.ccc>=miz.and.ccc<=mz) then
...
real(GLFLOAT)::color(4),calor(4)
  ...
  vx=vx-0.5d0*ux ; vy=vy-0.5d0*uy ; vz=vz-0.5d0*uz
  riv=riv+0.5d0*(r-riv)
...
integer(kind=glint), intent(in), value :: selection
real*8 :: guardaene
	  ...
	  read(*,*) k
          if (k>0) then
            do kk=1,cels(k)%nunodes
              ki=cels(k)%node(kk)
              print *,kk,"th node in cell",k,"is",ki
              oopp(kk)=ki
            end do
...
case(4)
        ...
        switch = .false.
case(5)
     	colorselection=21	!Tommi 10.1.2014
case(6)
...
case(17) 
         print *,"well, there is not point in changing that"
case(18) 
         print *,"well, there is not point in changing that"
case(19) 
         print *,"well, there is not point in changing that"
...
!*******************************************SUBROUTINE******************************************************** !<<<<<<Tommi 27.9.2013
subroutine pastemenu_handler(selection)
integer(kind=glint), intent(in), value :: selection
select case(selection)
case(1)
pasteselection=1
...
integer submenuidd,menuu,menud,menut,menuq,menus,menuse, menueditor, planemenu,nodemenu,cellmenu,nodecopymenu,cellcopymenu,pastemenu !Tommi 27.9.2013
...
do nu=1,nparam_per_node !>>>>>Tommi 1.8.2013
  call glutAddMenuEntry(nodeparams(nu)//char(0),nu)!>>>>>Tommi 1.8.2013
end do

call glutAddMenuEntry("as gene 1 concentration"//char(0),nu)!>>>>>Tommi 5.8.2013
call glutAddMenuEntry("as gene 2 concentration"//char(0),nu+1)
!call glutAddMenuEntry("as gene 3 concentration"//char(0),nu+2) Out of use because the menu window can only hold 43 items in the screen, and the same can be achieved with genex 
call glutAddMenuEntry("as gene x concentration"//char(0),nu+2)
call glutAddMenuEntry("as dx"//char(0),nu+3)
call glutAddMenuEntry("as dy"//char(0),nu+4)
call glutAddMenuEntry("as dz"//char(0),nu+5)
call glutAddMenuEntry("as dtotal"//char(0),nu+6)
call glutAddMenuEntry("as boxes"//char(0),nu+7)
call glutAddMenuEntry("as nodeid"//char(0),nu+8)!>>>>>Tommi 5.8.2013

menud = glutCreateMenu(menud_handler)
call glutAddMenuEntry("lines"//char(0),1)
call glutAddMenuEntry("boxes"//char(0),2)
...
call glutAddMenuEntry("draw whole triangulation"//char(0),37)
call glutAddMenuEntry("draw border faces"//char(0),38)
call glutAddMenuEntry("balls as radius=req"//char(0),5)
call glutAddMenuEntry("balls as radius=da"//char(0),6)
...
call glutAddMenuEntry("main box"//char(0),18)
call glutAddMenuEntry("polarization vectors"//char(0),19)
call glutAddMenuEntry("centroids"//char(0),21) ! miguel 30-5-13
call glutAddMenuEntry("hold"//char(0),36) !>> Miquel 15-1-14
call glutAddMenuEntry("movement unitary vectors"//char(0),23)     !>>>>Miquel 30-8-13
call glutAddMenuEntry("force components: cylinders"//char(0),24)  !
call glutAddMenuEntry("force components: tor"//char(0),25)        !
call glutAddMenuEntry("force components: stor"//char(0),26)       !
!call glutAddMenuEntry("force components: migration"//char(0),29)  !
...
call glutAddMenuEntry("save snap"//char(0),2)
...
call glutAddMenuEntry("save nice movie automatically (the window must be open and uncovered)"//char(0),10)
call glutAddMenuEntry("save fast movie automatically (the window dos not need to be open)"//char(0),12)
...
call glutAddMenuEntry("Paste the property to the selected node"//char(0),i) !<<<<<Tommi 23.9.2013
...
pastemenu = glutCreateMenu(pastemenu_handler)  !>>>>>Tommi 27.9.9.2013
call glutAddMenuEntry("Paste cell in original form"//char(0),1)
call glutAddMenuEntry("Paste cell and rotate cell 90 degrees clockwise"//char(0),2)!The menu to change the properties of a selected cell
call glutAddMenuEntry("Paste cell and rotate cell 180 degrees clockwise"//char(0),3)
call glutAddMenuEntry("Paste cell and rotate cell 270 degrees clockwise"//char(0),4) !<<<<<Tommi

menueditor = glutCreateMenu(menueditor_handler) !>>>>>Tommi !The editor main menu 27.9.2013
call glutAddSubMenu("Choose adding/selection plane"//char(0),planemenu) 
call glutAddMenuEntry("Add basic mesenchymal nodes"//char(0),1)
call glutAddMenuEntry("Move the embryo, left mouse button = pan"//char(0),2) 
call glutAddMenuEntry("Stop adding,selecting or moving"//char(0),3) 
call glutAddMenuEntry("Turn off the adding/selection plane"//char(0),4) 
call glutAddMenuEntry("Turn off the selection coloring (marge)"//char(0),5) 
call glutAddMenuEntry("Select a node"//char(0),6)
call glutAddMenuEntry("Paste a node from selection"//char(0),7)
call glutAddMenuEntry("Add basic epithelial cells"//char(0),8)
call glutAddMenuEntry("Select a cell"//char(0),9)
call glutAddSubMenu("Paste a cell from selection"//char(0),pastemenu)
...
call glutAddSubMenu("Copy properties of the selected node"//char(0),nodecopymenu)
call glutAddSubMenu("Copy properties of the selected cell"//char(0),cellcopymenu)!<<<<<Tommi 

menuid = glutCreateMenu(menu_handler)
call glutAddSubMenu("Basic View Modifier"//char(0),submenuid)
call glutAddSubMenu("color of balls"//char(0),menuu)
call glutAddSubMenu("what to draw"//char(0),menud)
call glutAddSubMenu("sections"//char(0),menut)
call glutAddSubMenu("read back"//char(0),menuq)
call glutAddSubMenu("input output"//char(0),menus)
call glutAddSubMenu("2D plots"//char(0),menuse)
call glutAddSubMenu("wake up the editor"//char(0),menueditor) !Tommi 5.8.2013
call glutAddMenuEntry("select node"//char(0),1)
call glutAddMenuEntry("select cell"//char(0),13)
call glutAddMenuEntry("move node"//char(0),2)
call glutAddMenuEntry("1 iteracio"//char(0),3)
call glutAddMenuEntry("100 iteracions"//char(0),4)
call glutAddMenuEntry("1000 iteracions"//char(0),5)
call glutAddMenuEntry("50000 iteracions"//char(0),6)
call glutAddMenuEntry("x iteracions"//char(0),7)
call glutAddMenuEntry("complexity"//char(0),15)
