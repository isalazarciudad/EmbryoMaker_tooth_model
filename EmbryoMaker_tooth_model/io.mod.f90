...
module io
  ...
  real*4,public  :: conf_xmin,conf_xmax,conf_ymin,conf_ymax,conf_zmin,conf_zmax
  integer::conf_dyn !>>Miquel5-11-14
...
!***************************************************************
  ...
  subroutine s_names_param
    ...
    names_param(30)="M(buoyancy)     : scale mod"
    names_param(31)="M()     : scale"
    names_param(32)="M()     : scale"
    names_param(33)="M()     : scale"
    names_param(34)="M()     : scale"
    names_param(35)="M(press)     : scale mod: parameter connecting mechanical stresses to cell cycle"
...
!**********************************************************************************************
  ...
  subroutine writeassuch(para,nodi)  !with nodeo
    ...
    if (ng>0) then 
      ...
      write (1,*) 
      do i=1,ng
        write(1,fmt="(A4,I4,A1,A50)") "gene",i ," ",gen(i)%name
    ...
    if(ffu(24)==1)then
      write (1,*) 
      write (1,*) "periodic borders"
      write (1,*) nborders
      do i=1,nborders
        write (1,"(4I5)") borders_neigh(i,1:4)
      end do
    end if
...
!print *,idum
    ...
    if (ng>0) then
      ...
      read(1,*,ERR=666,END=777)
      do i=1,ng
        read(1,"(A4,I4,A50)",ERR=666,END=777) duh,j ,gen(i)%name !!!ECTO MOD
    ...
    if(ffu(24)==1)then
      ...
      read (1,*)
      read (1,*) nborders
      allocate(borders_neigh(nborders,4))
      do i=1,nborders
        read (1,"(4I5)") borders_neigh(i,1:4)
      end do
    end if
...
!***************************************************************
  ...
  subroutine readsnapold(nofi)    !no nodeo
    ...
    if (ng>0) then
      ...
      read(1,*,ERR=666,END=777)
      do i=1,ng
        read(1,*,ERR=666,END=777) duh,j ,gen(i)%name !!!!ECTO MOD
        read(1,'(es24.16,A12)',ERR=666,END=777) gen(i)%diffu
        read(1,'(es24.16,A17)',ERR=666,END=777) gen(i)%mu
        read(1,'(es24.16,A36)',ERR=666,END=777) gen(i)%kindof
        read(1,*) gen(i)%npre !,"number of pre forms"
...
subroutine read_config_file !config file will set some general environmental variables for the model, like  !>>Miquel8-9-14
   ...
   read(7,*)
   read(7,*)conf_dyn
   read(7,*)conf_xmin,conf_xmax
   read(7,*)conf_ymin,conf_ymax
   read(7,*)conf_zmin,conf_zmax
   read(7,*)
...
subroutine no_config_file !>>Miquel2-1-14
   ...
   conf_dyn=1
   conf_xmin=-5000d0;conf_xmax=5000d0
   conf_ymin=-5000d0;conf_ymax=5000d0
   conf_zmin=-5000d0;conf_zmax=5000d0
