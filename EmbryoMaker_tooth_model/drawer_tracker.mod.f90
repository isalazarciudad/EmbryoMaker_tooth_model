...
subroutine inivisualitzacio
  ...
  flag(34)=conf_flag(34) !this is the switch to display the cell tracks !!!TRACKER MOD
  ...
  dyn=conf_dyn ; fix_run=0  !dynamic display box flag ; fixed dysplay box after running iterations flag !>>Miquel2-10-14 !>>Miquel11-11-16
  ...
  set_mix=conf_xmin ; set_mx=conf_xmax  !>>Miquel11-11-16
  set_miy=conf_ymin ; set_my=conf_ymax                   
  set_miz=conf_zmin ; set_mz=conf_zmax
...
real*8,dimension(:,:) :: vcl(2,nd) !point coordinates    !!!!!TRACKER MOD
!integer,dimension(:) :: vm(nd),ht(0:3/2*nd-1)
integer, dimension(:) :: ind(nd),stack(2*nd-1)
!integer,dimension(:,:) :: bf(1:3,nd**2),fc(1:7,nd**2) !point indices (the algorithm reorders)
integer,dimension(:,:):: til(3,nd**2),tnbr(3,nd**2) !!!!!TRACKER MOD
integer::maxst,ntri !!!!TRACKER MOD
...
!!print *,i,node(i)%altre,node(i)%tipus
  ...
  if(custom_colorselection==colorselection)then
    ...
    case(41) ; mine=minval(fmeanl(1:nd)) ; maxe=maxval(fmeanl(1:nd)) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(custom_arrowselection==arrowselection)then
   ...
   case(41) ; amine=minval(fmeanl(1:nd)) ; amaxe=maxval(fmeanl(1:nd)) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(custom_sphereselection==sphereselection)then
   ...
   case(41) ; smine=minval(fmeanl(1:nd)) ; smaxe=maxval(fmeanl(1:nd)) 
    case(42) ; smine=1 ; smaxe=nd
    end select
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!        !>>>Miguel30-10-14 
    if(sphereselection.gt.42)then
      schogen=arrowselection-42 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i=1,nd
    ...
    if(colorselection==29)then
      if(gex(i,11)>0.0.or.gex(i,26)>0.05.or.gex(i,27)>0.05)then
        color=0 ; color(1)=1
      !elseif(gex(i,26)>0.01.or.gex(i,27)>0.01)then
      !  color=0 ; color(2)=0.5
      !elseif(gex(i,19)>1d-1.or.gex(i,20)>1d-1.or.gex(i,21)>1d-1)then
      !  color=0 !; color(2)=0.5
      !  if(gex(i,19)>0.1)then; color(2)=0.5;end if
      !  if(gex(i,20)>0.1)then; color(3)=1.0 ; color(2)=1.0;end if
      !  if(gex(i,21)>0.1)then; color(1)=1.0 ; color(2)=0.5;end if
      !  !color=0 ; color(2)=0.5
      elseif(gex(i,1)>epsilod)then
        color=0 ; color(1)=0 ; color(3)=1.0
        !if(gex(i,19)<5d-2) color=color/2
      elseif(gex(i,2)>epsilod)then
        color=0 ; color(1)=0.75 ; color(2)=0.75
        !if(gex(i,20)<5d-2) color=color/2
      elseif(gex(i,3)>epsilod)then
        color=0 ; color(1)=0.5 ; color(3)=1
        !if(gex(i,21)<5d-2) color=color/2
      end if
    end if
...
!print*,"arrowselection",arrowselection
  ...
  if (flag(40)==1) then
    ...
    if ((flag(30)==1.and.gex(i,1)>0.1d0).or.(flag(31)==1.and.gex(i,3)>0.1d0).or.(flag(32)==1.and.gex(i,2)>0.1d0)) then
  ...
  !!!!!! DRAWING THE CELL TRAJECTORIES!!!!!! !!!TRACKER MOD
  if(flag(34)==1)then
    kk=80
    !print*,"tracker plotting"
    if(flag(7)==0)then
    call glBegin(gl_quads)
      do i=1,ncels
        ...
        if(node(k)%tipus==1.and.flag(9)==0) cycle
        if(node(k)%tipus==2.and.flag(10)==0) cycle
        if(gex(k,1)>0.1d0 .and.flag(30)==0) cycle
        if(gex(k,3)>0.1d0 .and.flag(31)==0) cycle
        if(gex(k,2)>0.1d0 .and.flag(32)==0) cycle
        if(flag(36)==0.and.node(k)%hold==1) cycle  !>>>>Miquel15-1-14
      
        if(track_length>1)then
          a=i
          b=ncels
          call get_rainbow(a,1.0_gldouble,b,color)
          !do j=track_length-d,track_length-1
          do j=1,track_length-1
            !if(j<1)then;jj=1;else;jj=j;end if
            !print*,"jj",jj,"d",d
            !print*,"j",j,"track_length",track_length,"kk",kk
            if(j<track_length-kk) cycle
            a=cell_track(i,j,1) ; b=cell_track(i,j,2) ;
            if (a>=mix.and.a<=mx.and.b>=miz.and.b<=mz) then
              aa=cell_track(i,j+1,1) ; bb=cell_track(i,j+1,2) ;
              if (aa>=mix.and.aa<=mx.and.bb>=miz.and.bb<=mz) then
                color=1

                aaa=(bb-b) ; bbb=-(aa-a) ; d=sqrt(aaa**2 + bbb**2) ; aaa=a+0.05*aaa/d ; bbb=b+0.05*bbb/d


                !normal=normcrossprod((/a,0.0_gldouble,b/),(/aa,0.0_gldouble,bb/),(/aaa,0.0_gldouble,bbb/))
                !normal=normcrossprod((/aa,0.0_gldouble,bb/),(/a,0.0_gldouble,b/),(/aaa,0.0_gldouble,bbb/))  
                !call glnormal3fv(normal)
                !call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)

                !call glvertex3d(esca*a,0.0_gldouble,esca*b)
                !call glvertex3d(esca*aa,0.0_gldouble,esca*bb)
                !call glvertex3d(esca*aaa,0.0_gldouble,esca*bbb)

                aaaa=(bb-b) ; bbbb=-(aa-a) ; d=sqrt(aaaa**2 + bbbb**2) ; aaaa=aa+0.05*aaaa/d ; bbbb=bb+0.05*bbbb/d

                !normal=normcrossprod((/aaaa,0.0_gldouble,bbbb/),(/aaa,0.0_gldouble,bbb/),(/aa,0.0_gldouble,bb/))
                !normal=normcrossprod((/aaa,0.0_gldouble,bbb/),(/aaaa,0.0_gldouble,bbbb/),(/aa,0.0_gldouble,bb/))  
                !call glnormal3fv(normal)
                !call glvertex3d(esca*aa,0.0_gldouble,esca*bb)
                !call glvertex3d(esca*aaa,0.0_gldouble,esca*bbb)
                !call glvertex3d(esca*aaaa,0.0_gldouble,esca*bbbb)


                !if(b>bbb)then
                !  normal=normcrossprod((/a,0.0_gldouble,b/),(/aa,0.0_gldouble,bb/),(/aaa,0.0_gldouble,bbb/))
                !else
                !  normal=normcrossprod((/aaa,0.0_gldouble,bbb/),(/aa,0.0_gldouble,bb/),(/a,0.0_gldouble,b/))
                !end if
                !normal=normcrossprod((/aa,0.0_gldouble,bb/),(/a,0.0_gldouble,b/),(/aaa,0.0_gldouble,bbb/))  
                !call glnormal3fv(normal)
                call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
       ...
       !call glTranslated(a*esca,0.0_gldouble,b*esca)
       !call glutsolidsphere(0.01_gldouble, 10, 10)
       !call glTranslated(-a*esca,0.0_gldouble,-b*esca)
   ...
   !print*,"hello!!!"
     ...
     !print*,"k",k
        ...
        if(track_length>1)then
          ...
          do j=1,track_length-1
            !if(j<1)then;jj=1;else;jj=j;end if
            if(j<track_length-kk) cycle
            a=cell_track(k,j,1) ; b=cell_track(k,j,2) ;
            !print*,"node(K)%icel",node(k)%icel

            if (a>=mix.and.a<=mx.and.b>=miz.and.b<=mz) then
              aa=cell_track(k,j+1,1) ; bb=cell_track(k,j+1,2) ;
              if (aa>=mix.and.aa<=mx.and.bb>=miz.and.bb<=mz) then
                call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
                call glvertex3d(esca*a,0.0_gldouble,esca*b)
                call glvertex3d(esca*aa,0.0_gldouble,esca*bb)
    ...
    call glEnd
       !a=node(k)%x ; b=node(k)%y ; c=-node(k)%z !; print*,"abc",a,b,c
       color=1 ; !color(1)=1 ;
       call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)

       r=0.05d0*esca
       call glTranslated(aa*esca,0.0_gldouble,bb*esca)
       call glutsolidsphere(r, 10, 10)
       call glTranslated(-aa*esca,0.0_gldouble,-bb*esca)
  ...
  if(flag(37)==1)then !DRAWING OF TISSUE SURFACE BY 2D TRIANGULATION !!!!!TRACKER MOD
    npt=nd
    !sizht=3/2*npt
    !maxfc=npt**2
    !maxbf=npt**2
  ...
  !print*,i,"fc",fc(:,i)
      if(til(1,i)>0.and.til(2,i)>0.and.til(3,i)>0)then !a valid triangle
        ...
        !end if
        !cycle
        if (a>=mix.and.a<=mx.and.c>=miz.and.c<=mz) then
          if (aa>=mix.and.aa<=mx.and.cc>=miz.and.cc<=mz) then
            if (aaa>=mix.and.aaa<=mx.and.ccc>=miz.and.ccc<=mz) then
...
real(GLFLOAT)::color(4),calor(4)
  ...
  if(flag(11)==1)then; r=r*0.3d0 ;riv=riv*0.3d0; end if
...
subroutine cylinder2(nod,iv,res,color)  !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Miquel 15-5-13
integer::nod,iv,res,i,j,k,ii,jj,kk
real*8::r,riv,cx,cy,cz,ix,iy,iz,vx,vy,vz,d
real*8::rad,ax,ay,az,ux,uy,uz,thet
real*8::bx,by,bz,ox,oy,oz,cost,sint,ucost
...
print*,"colorselection",selection
...
case(16)
        ...
        call pastecell(cursx,cursy,cursz)
case(17)
        print*,"ATTENTION the change will be applied to all selected nodes"
        flag(40)=1
        print *,"which gene"
        read(*,*) qgen
...
real*8         :: disx(nd),disy(nd),disz(nd),distot(nd)
  ...
  !BALL COLORS MAX AND MINS
    ...
    case(32) ; mine=minval(node(:nd)%marge) ; maxe=maxval(node(:nd)%marge) 
    case(33) ; mine=minval(node(:nd)%talone) ; maxe=maxval(node(:nd)%talone) 
    case(34) ; mine=0;maxe=2!minval(node(:nd)%hold) ; maxe=maxval(node(:nd)%hold) 
    case(35) ; mine=0;maxe=1
               !mine=minval(node(:nd)%border) ; maxe=maxval(node(:nd)%border) 
...
!!!! COLOR MENUS  !>>Miquel30-10-14
  ...
  call glutAddMenuEntry("as mechanical stress"//char(0),41)
  call glutAddMenuEntry("as node index"//char(0),nu+7)!>>>>>Tommi 5.8.2013
  do i=1,ng						!>>>Miguel30-10-14
    write(genetitle,*),i," ",gen(i)%name !!!ECTO MOD  !>>>Miguel30-10-14
...
call glutAddMenuEntry("cell tracks"//char(0),34)      !
