...
integer  ::alone              ! 0 if the node is really alone
  ...
  if(allocated(fneigh)) deallocate(fneigh)  !!!MECHVEC MOD
  allocate(fneigh(nd,mnn)) ; fneigh=0d0  !!!MECHVEC MOD
  ...
  do nod=1,nd
    ...
    if(tipi<3)then
      iv=node(nod)%altre
      ax=node(iv)%x   ; az=node(iv)%z
      cx=ax-ix        ; cz=az-iz
      dd=sqrt(cx**2+cz**2)
      udd=1d0/dd
      !if(ffu(1)==1)then
        uvx=cx*udd ; uvz=cz*udd  !the unit vector
        ddd=dd-node(nod)%reqs-node(iv)%reqs  !>>Miquel5-2-14
        f=2*node(nod)%ke*ddd    !the force
        rsprx=f*uvx ; rsprz=f*uvz
    ...
    do i=1,nneigh(nod)
      ...
      if (tipi<3)then
        if(tipic<3)then
          ivv=node(ic)%altre
          icx=node(ivv)%x-bx ; icz=node(ivv)%z-bz 
          idd=sqrt(icx**2+icz**2)      ; iudd=1d0/idd	!ic's spring vector	
          posca=icx*cx+icz*cz
          if (tipi==tipic) then           ! BOTH NODES ARE EPITHELIAL IN THE SAME SIDE we are equal so we must have lateral adhesion
            if (posca>epsilod) then    !SO WE THE NEIGHBOUR IS IN A CONTIGUOUS PART OF THE EPITHELIUM
              ...
              mcx=icx+cx; mcz=icz+cz; md=1d0/sqrt(mcx**2+mcz**2)  !we take as spring vector the sum of ic's and nod's spring vectors
              dotp=(mcx*ccx+mcz*ccz)
              ...
              pesco=dotp*md**2
              uvx=(ccx-mcx*pesco) ; uvz=(ccz-mcz*pesco)  !unit vector of the within cilinder force !el modul és el mateix que el vector c
              ...
              if(ffu(16)==0)then
                mcx=icx+cx; mcz=icz+cz; md=sqrt(mcx**2+mcz**2)  !we take as spring vector the sum of ic's and nod's spring vectors
                if(md<epsilod)then !the two cylinders are parallel, the vector used is the spring !>>Miquel20-8-14
                  ...
                  if (dotp<epsilod) then         ! projection of the vector from nod to ic into the vector from nod to iv
                    ...
                    if (ddd-a<epsilod) then
                      ...
                      uvx=-cx*udd ; uvz=-cz*udd  !unit vector vertical
          ...
          dotp=(cx*ccx+cz*ccz)!;print*,"dotp epi-mes",dotp,nod,ic !hi ha un vector del revés, per tant això està al revés també
          if (dotp<0.0) then
        ...
        if(tipic<3)then          ! IC IS EPITHELIAL and nod is not
          ...
          icx=node(ivv)%x-bx ; icz=node(ivv)%z-bz   
          idd=1d0/sqrt(icx**2+icz**2)
          dotp=(icx*ccx+icz*ccz)  !hi ha un vector del revés, per tant això està al revés també
          if (dotp>0.0) then
            ...
            if (ddd-a<0.0) then
              ...
              uvx=icx*idd ; uvz=icz*idd  !unit vector vertical
              fd=ddd     !distance used, vertical component
          ...
          uvx=ccx*ud ; uvz=ccz*ud  !unit vector of the within cilinder force !el modul és el mateix que el vector c
...
300   nuve=nuve+1
      ...
      !if (ffu(2)==1) then IS 25-12-13
        if(node(nod)%icel==node(ic)%icel)then
          ...
          if(fd-deqe<-epsilod)then 
            ...
            rtorx(nod)=rtorx(nod)+f*uvx ; rtory(nod)=rtory(nod)+f*uvy ; rtorz(nod)=rtorz(nod)+f*uvz  !>>MECHVEC MOD
          else
            if(twoep/=2)then   !!SCALE MOD. prevent apical sides of the epith. to stick when the epith. is folded
              ...
              rstorx(nod)=rstorx(nod)+f*uvx ; rstory(nod)=rstory(nod)+f*uvy ; rstorz(nod)=rstorz(nod)+f*uvz  !>>MECHVEC MOD
            end if
          end if
        end if !;print*,"f",f,"fd",fd
        !vforce(nuve,nod)=f     !>>>>>Miquel 7-8-13
        !if (twoep/=2) vforce(jjjj,ic)=f      !>>>>>Miquel 7-8-13
        rcilx(nod)=rcilx(nod)+f*uvx ; rcilz(nod)=rcilz(nod)+f*uvz  !>>>Miquel 7-8-13
        rcilx(ic)=rcilx(ic)-f*uvx ; rcilz(ic)=rcilz(ic)-f*uvz  !>>>Miquel 4-4-14
        fneigh(nod,i)=f !; fneigh(ic,nod)=f !!!MECHVEC MOD
        !if(tipi<3 .and. tipic<3)then 
           
        !fmeanl(nod)=fmeanl(nod)+(fd-deqe) ; fmeanl(ic)=fmeanl(ic)+(fd-deqe) ! >>> Is 21-6-14
        fmeanl(nod)=fmeanl(nod)+f ; fmeanl(ic)=fmeanl(ic)+f ! >>> Is 21-6-14
     ...
     !if(nod==260.or.ic==260) print*,"dzero",nod,ic,fd
      ...
      if(ffu(4)==0 .and.twoep==1 ) then !it is only between epithelial nodes
        ...
        mcx=icx+cx; mcz=icz+cz; md=1d0/sqrt(mcx**2+mcz**2)  !we take as spring vector the sum of ic's and nod's spring vectors
        dotp=(mcx*ccx+mcz*ccz)*md !vertical projection, more stable than the angle
        ...
        if(abs(dotp)-angletor*d>epsilod)then
          uvx=mcx*md ; uvz=mcz*md
          f=(tornod+node(ic)%stor)*dotp         !>>>>>Miquel 7-8-13
          !rstorx(nod)=rstorx(nod)+f*uvx ; rstory(nod)=rstory(nod)+f*uvy ; rstorz(nod)=rstorz(nod)+f*uvz !>>MECHVEC MOD
          !rstorx(ic)=rstorx(ic)-f*uvx ; rstory(ic)=rstory(ic)-f*uvy ; rstorz(ic)=rstorz(ic)-f*uvz !>>MECHVEC MOD
          ...
          dotp=-(icx*ccx+icz*ccz)*iudd !vertical projection, more stable than the angle
          f=(tornod+node(ic)%tor)*dotp
          !rtorx(ic)=rtorx(ic)-f*uvx ; rtory(ic)=rtory(ic)-f*uvy ; rtorz(ic)=rtorz(ic)-f*uvz !>>MECHVEC MOD
        end if
...
789 if (ffu(8)==1) then   !this is kind of crappy in the sense that 
    ...
    if (aut==0) then ! if aut==0 there is not display   
      vsprx(nod)=rsprx ; vsprz(nod)=rsprz                     !putting the force components into storage vectors for display
      vcilx(nod)=rcilx(nod)    ; vcilz(nod)=rcilz(nod)         !this part should be turned down
      vtorx(nod)=rtorx(nod)    ; vtorz(nod)=rtorz(nod)         !when there is no display
      vstorx(nod)=rstorx(nod)  ; vstorz(nod)=rstorz(nod)!
    end if
    ...
    if(node(nod)%hold==1)then
      ...
      d=uvx**2+uvz**2 ; if(d>epsilod)then ; ud=1d0/sqrt(d);else;ud=0;end if
      a=node(nod)%khold
    ...
    dex(nod)=sqrt(rvx**2+rvz**2)  !module of the force vector
    px(nod)=rvx ; pz(nod)=rvz
  end do
