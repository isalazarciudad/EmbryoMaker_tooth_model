...
integer,dimension(nd)::sorted_list

integer::nrects
real*8::posix,posiy,posiz,vhx,vhy,vhz,vwx,vwy,vwz,hei
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i=1,nd-1
    do j=i+1,nd
      if(node(sorted_list(i))%y < node(sorted_list(j))%y)then
        ii=sorted_list(i) ; sorted_list(i)=sorted_list(j)
        sorted_list(j)=ii
      end if
    end do
  end do
  ...
  do ii=1,nd
    ...
    if(chogen==26.and.node(i)%tipus==3) a=gex(i,27)
    if(chogen==5.and.node(i)%z>-1d0) a=0d0
    ...
    !if(colorselection==41) call get_rainbow(a,-4d0,4d0,color)

    if(colorselection==29)then
      if(gex(i,11)>0.0.or.gex(i,26)>0.05.or.gex(i,27)>0.05)then
        color=0 ; color(1)=1
      elseif(gex(i,26)>0.05.or.gex(i,27)>0.05)then
        color=0 ; color(2)=0.5
      !elseif(gex(i,19)>5d-1.or.gex(i,20)>5d-1.or.gex(i,21)>5d-2)then
      !elseif(gex(i,21)>1d-1)then
        !color=0 !; color(2)=0.5
        ...
        !if(gex(i,19)>0.5)then; color(2)=0.5;end if
        !if(gex(i,20)>0.5)then; color(3)=1.0 ; color(2)=1.0;end if
        !if(gex(i,21)>0.05)then; color(1)=1.0 ; color(2)=0.5;end if



      !  if(gex(i,19)>0.05)then; color(2)=0.5;end if
      !  if(gex(i,20)>0.05)then; color(3)=1.0 ; color(2)=1.0;end if
      !  if(gex(i,21)>0.05)then; color(1)=1.0 ; color(2)=0.5;end if
      ...
      elseif(gex(i,1)>epsilod)then
        ...
        !if(gex(i,23)>epsilod)then;color=0 ; color(1)=0 ; color(3)=1.0;end if
      ...
      elseif(gex(i,2)>epsilod)then
        ...
        !color(3)=1.0 ; color(2)=1.0
        !if(gex(i,23)>epsilod)then;color=0 ; color(1)=0.75 ; color(2)=0.75;end if
    ...
    if(chogen==5 .and. a>epsilod)then
      if(gex(i,1)>epsilod)then
        color=0; color(1)=1d0 ; color(2)=0.5
      elseif(gex(i,2)>epsilod)then
        color=0; color(2)=1d0
      elseif(gex(i,3)>epsilod)then
        color=0; color(2)=1d0 ; color(3)=1d0
      end if
    end if
      ...
      if ((flag(9)==1.and.node(i)%tipus==1).or.(flag(10)==1.and.node(i)%tipus==2) .or.(node(i)%tipus>=3)) then      
        a=node(i)%x ; b=node(i)%y ;  c=node(i)%z      
           ...
           if(chogen==5)then
              ...
              if (gex(i,5)>epsilod.and.node(i)%z<-1d0)then
                !color=0 ; color(2)=1d0
                color(4)=gex(i,5)/10d0
              else
                color(4)=0.01d0
              end if
            end if
...
!!!!!!!DRAW A HEATMAP SCALE
if(1==2)then
  nrects=10
  hei=5d0
  posix=0d0 ; posiy=0d0 ; posiz=0d0
  vhx=0d0 ; vhy=0d0 ; vhz=0.1d0
  vwx=0.1d0 ; vwy=0.0d0 ; vwz=0d0
  do i=1,nrects-1
    a=real(i-1)/real(nrects) ; b=real(i)/real(nrects)

    p1x=posix+vhx*a*hei ; p1y=posiy+vhy*a*hei ; p1z=posiz+vhz*a*hei
    p2x=posix+vhx*a*hei+vwx ; p2y=posiy+vhy*a*hei+vwy ; p2z=posiz+vhz*a*hei+vwz

    p3x=posix+vhx*b*hei ; p3y=posiy+vhy*b*hei ; p3z=posiz+vhz*b*hei
    p4x=posix+vhx*b*hei+vwx ; p4y=posiy+vhy*b*hei+vwy ; p4z=posiz+vhz*b*hei+vwz

    call glBegin(gl_triangles)
       !!!!!!!!!!!!!!!!!!!!!
       call get_rainbow(a,0d0,1d0,color)
       call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
       call glvertex3d(p1x,p1y,p1z)
       call glvertex3d(p2x,p2y,p2z)
       call get_rainbow(b,0d0,1d0,color)
       call glMaterialfv(gl_front_and_back,gl_ambient_and_diffuse,color)
