...
subroutine initiate_gene  ! allocates the matrices and fills npag and whonpag
  ...
  gen(:)%name="Regulatory molecule"  !!!ECTO MOD
  gen(:)%npre=0
  gen(:)%npost=0
  gen(:)%diffu=0.0d0
  gen(:)%idiffu=0.0d0
...
!      rdiffmax=2*maxval(node(:nd)%da)*dmax !now it's in the iteracio subroutine !>>Miquel27-2-14
      do i=1,nd
        ...
        !if(node(i)%border/=0)then
        !  kk=node(i)%border
        !  do i1=1,4
        !    ie=borders_neigh(kk,i1)
        !    if(ie==0) exit
        !    dist=node(i)%req+node(ie)%req
        !    !udist=1.0d0/(dist)
        ...
        do i1=1,nneigh(i)
          ...
          if (nkindof(4)>0) then
            if (dist<nbh) then
              do jjj=1,nkindof(4) ! kindof 4 diffuses within the cell but it is only in its surface (so it is not within the cell really)
                j=wkindof(4,jjj)
!ECTO MOD       !if ((node(ie)%tipus==1 .and. tipi==2).or.(node(ie)%tipus==2 .and. tipi==1)) then 
                  !did(j)=did(j)+gen(j)%diffu*(gex(ie,j)-gex(i,j))/(dist+1) no diffusion across epitelia for signaling molecules
                  !did(j)=did(j)+gen(j)%diffu*(gex(ie,j)-gex(i,j))*udist 
!ECTO MOD       !else 
                  did(j)=did(j)+gen(j)%diffu*(gex(ie,j)-gex(i,j))*udist !;print*,"i,ie",i,ie,"udist",udist
!ECTO MOD       !end if
          ...
          if (nkindof(9)>0) then
            if (dist<nbh) then
              do jjj=1,nkindof(9) ! kindof 4 diffuses within the cell but it is only in its surface (so it is not within the cell really)
                j=wkindof(9,jjj)
!ECTO MOD       !if ((node(ie)%tipus==1 .and. tipi==2).or.(node(ie)%tipus==2 .and. tipi==1)) then 
                  !did(j)=did(j)+gen(j)%diffu*(gex(ie,j)-gex(i,j))/(dist+1) no diffusion across epitelia for signaling molecules
                  !did(j)=did(j)+gen(j)%diffu*(gex(ie,j)-gex(i,j))*udist 
...
!ECTO MOD       !end if
        ...
        if(node(i)%border==1 .and. (nkindof(4)>0 .or. nkindof(9)>0))then
          do jjj=1,nkindof(4) ! kindof 4 diffuses within the cell but it is only in its surface (so it is not within the cell really)
            j=wkindof(4,jjj)
        ...
        if (node(i)%marge==0.and.node(i)%tipus<4) then ! IN THE NUCLEUS (THERE WILL BE TRANSCRIPTION)
          do jjj=1,nkindof(1)           !*****************PRODUCTION OF PRIMARY FORMS (WITHOUT FURTHER FORMS)
            ...
            do k=1,int(nw(j))
              ...
              if((gen(kk)%kindof==4 .or. gen(kk)%kindof==9) .and. gen(kk)%npre>0)then
                recep=0
                do kki=1,gen(kk)%npre   
                  kkk=gen(kk)%pre(kki)
                  if(gex(i,kkk)>epsilod) recep=recep+1
                end do
                if(recep==0) cycle
              end if
              !if(i==814 .and. j==5) print*,"kk",kk,"w",gen(j)%w(kk),gex(i,kk)
            ...
            !if(i==814 .and. j==5) print*,"sm",sm,"a",a,"gext",gext(j)
          end do 
          ...
          do jjj=1,nkindof(9)           !*****************PRODUCTION OF PRIMARY FORMS (WITHOUT FURTHER FORMS)
            j=wkindof(9,jjj)     
            a=gex(i,j)
            sm=0.0d0 ; smn=0.0d0
            do k=1,int(nw(j))
              ...
              !crappy implementation of signal receptors
              !sm=sm+gen(j)%w(kk)*gex(i,kk)
              aa=gen(j)%w(kk)
              if(aa>=epsilod)then
                !if((j==2 .and. node(i)%tipus==3).or.(j==9 .and. node(i)%tipus<3))then
                  sm = sm + aa*gex(i,kk)**2  !the amount of TFs
                !sm = sm + aa*gex(i,kk)  !the amount of TFs
                !end if
              else
                smn = smn + abs(aa)*gex(i,kk)  !the amount of TFs
              end if
            end do
            !if(sm<-epsilod) sm=0.0d0
            !if(j==2)then
              !if (i==1) print*,"pre j",j,"gext(j)",sm/(1+smn)
            
              gext(j) = gext(j) + sm/(1+smn) !- gen(j)%mu*a + did(j) !gain of product by transcription (Michaelis-Menten) !>>>> Miquel 20-12-13
 
              !if (i==1) print*,"1 j",j,"gext(j)",gext(j),"deg",gen(j)%mu*a,"did",did(j)

            !elseif(j==9)then
              !gext(j) = gext(j) + sm - gen(j)%mu*a + did(j) !gain of product by transcription (Michaelis-Menten) !>>>> Miquel 20-12-13
            !end if
            !gext(j) = gext(j) + sm- gen(j)%mu*a**2 + did(j) !gain of product by transcription (Michaelis-Menten) !>>>> Miquel 20-12-13
            !gext(j) = gext(j) + sm -gen(j)%mu*a + did(j) !gain of product by transcription (Michaelis-Menten) !>>>> Miquel 20-12-13
          ...
          !    kk=w(j,k)
          !    aa=gen(j)%w(kk)
          !    if(gen(kk)%npre>0)then
          !      do kki=1,gen(kk)%npre   
          !        kkk=gen(kk)%pre(kki)
          !        if(gex(i,kkk)>epsilod)then
          !          if(aa>=epsilod)then
          !            sm = sm + aa*(gex(i,kk)**2)  !the amount of TFs
          !          else
          !            smn = smn + abs(aa)*gex(i,kk)  !the amount of TFs
        ...
        do jjj=1,nkindof(4)           ! THIS IS IMPORTANT, IF YOU WANT TO MAKE A NOTCH YOU SHOULD MAKE ITS PREVIOUS FORM AND TRANSCRIBE IT
          ...
          ! NOTICE: growth factors do not return any thing to pre forms, there is not back reaction, because they get secreted as they get produced
            !if(i==1.and.j==1) print*,"passa per aqui"

          gext(j) = gext(j)  - gen(j)%mu*a + did(j)  ! Is 9-2-14
        end do
        do jjj=1,nkindof(9)           ! THIS IS IMPORTANT, IF YOU WANT TO MAKE A NOTCH YOU SHOULD MAKE ITS PREVIOUS FORM AND TRANSCRIBE IT
          j=wkindof(9,jjj)     
          a=gex(i,j)
          if (gen(j)%npost>0) then 
            do kki=1,gen(j)%npost   
          ...
          ! NOTICE: growth factors do not return any thing to pre forms, there is not back reaction, because they get secreted as they get produced
            !if(i==1.and.j==1) print*,"passa per aqui"
          gext(j) = gext(j)  - gen(j)%mu*a + did(j)  ! Is 9-2-14
          !if (i==1) print*,"2 j",j,"gext(j)",gext(j),"deg",gen(j)%mu*a,"did",did(j)
        end do 
        ...
        do jjj=1,nkindof(8)
         ...
         !if(i==1) print*,"j2 gextfinal",gext(2)
         !if(i==1) print*,"j9 gextfinal",gext(9)
         
         
        dgex(i,:)=gext  ! this is the increment in the gene this step
        !if(i==1) print*,"dgex",dgex(i,:)
      end do
