...
!!!!!!GENETIC VARIABLES!!!!!!
type(dev_genes),device,allocatable,dimension(:) ::d_gen
...
integer,device              ::d_nkindof(8)   ! number of each kindof gene
    ...
    do i2=-nbo,nbo
      ...
      do i3=-nbo,nbo
        ...
        do while(ie.ne.0)
          ...
          if (ie==ivv) then ; ie=d_list(ie) ; cycle ; end if
          if (tipi<3 .and. d_node(ie)%tipus/=tipi) then ; ie=d_list(ie) ; cycle ; end if !>>> Is 18-4-15
          dist=sqrt((d_node(ie)%x-ix)**2+(d_node(ie)%y-iy)**2+(d_node(ie)%z-iz)**2)
          !>>Miquel31-12-14
          a=dai+d_node(ie)%da
          if(tipi>=3)then  
...
789 if (d_ffu(8)==1) then   !this is kind of crappy in the sense that 
    ...
    if(d_node(nod)%hold==1)then
      ...
      d=uvx**2+uvy**2+uvz**2 ; if(d>d_epsilod)then ; ud=0;end if
      a=d_node(nod)%khold
